#-----------------------------------------------------------------------------#
# Program: StateASDR_40-84_AllCoD_Figures.r                                   #
# Description:	*** All CoD combined ***										                  #
...
col.reg <- c(col.reg1, col.reg2, col.reg3, col.reg4, col.reg5, col.reg6, col.reg7,
             col.reg8, col.reg9)


# Plots: All causes of death combined
# -----------------------------------
...
ymin <- ifelse(sex.id=="m", 11, 6)
ymax <- ifelse(sex.id=="m", 44, 30)
by.y <- 4 ## Distance between tick marks on the y-axis
lev.reg.x <- xmax + 1.5 ## x-location of region's names
lev.reg.y <- ymax - 1.5 ## y-location of region's names

dev.off()
pdf(paste("ASDR4084_AllCoD_USStates_", sex.id, ".pdf", sep=""),
    width=8.5, height=11)
source(file="~/OneDrive - Universite de Montreal/Projet_USStates_Mortality/USStates_CVRev/Rprograms/Fig_StatesByRegion.r")
#title(paste("ASDR 40-84, all causes of death combined, ",
