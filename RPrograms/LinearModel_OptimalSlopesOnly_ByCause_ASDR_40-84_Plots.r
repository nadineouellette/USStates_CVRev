#-------------------------------------------------------------------------------#
# Program: LinearModel_OptimalSlopesOnly_ByCause_ASDR_40-84_Plots.r				#
# Description:	*** By cause of death ***										#
#				For each sex, plot cause-specific age-standardized death rates	#
#				and fitted regression lines (ages 40-84) for all U.S. states by	#
#				region. The fitted regression lines (two slopes or three		#
#				slopes) were selected according to BIC and the p-values of the 	#
#				estimated regression parameters.								#
# Name: Nadine Ouellette														#
# Date: December 18 2014														#
#		(adapté de 'LinearModel_OptimalSlopes_ByCause_ASDR_40-84_Plots.r')		#
# Modified: *February, 3 2015* (Respecter l'ordre alphabétique des États dans	#
#			chaque région; modifier l'échelle des ordonnées pour les chez les	#
#			hommes (2 digits seulement); rectifier l'emplacement du titre de	#
#			certaines régions pour certaines combiaisons sexe/cause)			#
#			*February 10, 2015* (Avec les données pour les années 2005 à 2010)	#
#			*February 24, 2016* (Figures pour CVDs)								#
#-------------------------------------------------------------------------------#
...
load(file="~/Documents/Projet_USStates_Cancer/Data/ROutputs/LinearModel_ByCause_ASDR_40-84_CVDs.RData")
...
load(file="~/Documents/Projet_USStates_Cancer/Data/ROutputs/LinearModel_3Slopes_ByCause_ASDR_40-84_CVDs.RData")
...
dev.off()
pdf(paste("ASDR40-84_AllCOD_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
#pdf(paste("ASDR40-84_AllCOD_USStates_", sex.id, "_RegLine_OptSlopesOnly_NoTitle.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
title(paste("ASDR 40-84, all causes of death combined, ",sex.id,"s",sep=""),line=-1.15,cex.main=1.5,font.main=2,outer=TRUE)
dev.off()

# Figures for all CVDs
cause <- 7
leg.loc <- "bottomleft" ## Legend location on graphs
csasdr <- strat4084[,,sex.id,1] + strat4084[,,sex.id,2] # Heart + Other CVDs
ymin <- ifelse(sex.id=="male", 2, 1)
ymax <- ifelse(sex.id=="male", 24, 14.5)
by.y <- ifelse(sex.id=="male", 4, 1.5) ## Distance between tick marks on the y-axis
lev.reg.y <- ifelse(sex.id=="male", 23, 13.8) ## y-location of region's names
lev.reg.x <- xmax+1.5 ## x-location of region's names
pos.reg <- 2

dev.off()
pdf(paste("ASDR40-84_CVDs_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
title(paste("ASDR 40-84, cardiovascular diseases, ",sex.id,"s",sep=""),line=-1.15,cex.main=1.5,font.main=2,outer=TRUE)
dev.off()
...
dev.off()
pdf(paste("ASDR40-84_HeartDis_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
...
dev.off()
pdf(paste("ASDR40-84_OtherCVDs_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
...
dev.off()
pdf(paste("ASDR40-84_SmkMN_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
...
dev.off()
pdf(paste("ASDR40-84_OtherMN_USStates_", sex.id, "_RegLine_OptSlopesOnly.pdf", sep=""),width=8.5,height=11)
source(file="~/Documents/Projet_USStates_Cancer/Rprograms/Fig_StatesByRegion_RegressionLines_OptimalSlopesOnly.r")
